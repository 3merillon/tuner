...
Notes.prototype.updateFrequencies = function () {
	this.$notes.forEach(($note) => {
		const noteValue = parseInt($note.dataset.value);
		const frequency = this.tuner.getStandardFrequency(noteValue);
		$note.dataset.frequency = frequency.toFixed(2);
	});
};
